...
export default function HomeAlbums() {
  ...
  const { data: RECOMMENDED_ALBUMS, isLoading, isError, isSuccess, refetch } = useQuery({
    queryKey: ['RECOMMENDED_ALBUMS'],
    ...
    <HomeLayout page="home-library" title="Favorite singers">
      ...
      <article>
        ...
        <div className="content" style={{ gridTemplateColumns: `repeat(${columnCount}, minmax(0, 1fr))`, }} >
          {RECOMMENDED_ALBUMS.map((item, index) => (
            <div className='card' key={index} onClick={() => router.push(`/album/${item._id}`)} >
              <div className="images">
                <Image
                  ...
                  height={200}
                  placeholder="blur"
                  blurDataURL="/other/unknown.user.music.webp"
