...
import { useSearch, useStore } from "../../store/zustand";
import { FiChevronLeft, FiChevronRight } from "react-icons/fi";

export default function MainHeader() {
    const setNewsPath = useStore((state) => state.setNewsPath);
    const newsPath = useStore((state) => state.newsPath);
    ...
    return (
        <header id="main-header">
            <div className="naviga">
                <Link href={`/${back()}`} name="back" aria-label="Back Path"><FiChevronLeft /></Link>
                <div className="path">
            ...
            <div className="manage">
                ...
                <Link
                    href={pathname == '/news' ? newsPath : '/news'}
                    className={pathname && pathname.split('/')[1] == "news" ? "active" : ""}
                    onClick={() => setNewsPath(pathname)}
                    aria-label="Notification News"
                    name="news"
                >
