...
export default function Album() {
  ...
  const fetchData = () => {
    ...
    setCurrentData(albums.slice(startIndex, endIndex));
  };
  ...
  useEffect(() => {
    setTotalPages(Math.ceil(albums.length / perPage));
    // setSlicedData([]);
    // setPage(1);
    fetchData();
  }, [albums]);
  ...
  return (
    <section
      className={sidebar ? "adout albums" : "adout albums active"}
      ref={scrollRef}
    ...
    >
      <header>
        <div className="category">
          ...
          <h3>/</h3>
          <h2>{t("admin.album.title")}</h2>
        </div>
        <div className="others">
          <div className="search">
            <input
              type="text"
              placeholder={t("admin.album.input")}
              onKeyUp={(e) => setSearchInput(e.target.value)}
            />
            <HiSearch />
          </div>
          <div className="addUser">
            <button onClick={()=>navigate("/admin/musics/albums/add-album")}>{t("admin.album.button")}</button>
          </div>
        </div>
      </header>
      <div className="content">
        <InfiniteScroll
          dataLength={albums.length} // Malumotlar massivi uzunligi
