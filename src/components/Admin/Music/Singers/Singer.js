...
export default function Singer() {
  ...
  const fetchData = () => {
    ...
    setCurrentData(singers.slice(startIndex, endIndex));
  };
  ...
  useEffect(() => {
    setTotalPages(Math.ceil(singers.length / perPage));
    // setSlicedData([]);
    // setPage(1);
    fetchData();
  }, [singers]);
  ...
  return (
    <section
      className={sidebar ? "adout singers" : "adout singers active"}
      ref={scrollRef}
    ...
    >
      <header>
        <div className="category">
          ...
          <h3>/</h3>
          <h2>{t("admin.singer.title")}</h2>
        </div>
        <div className="others">
          <div className="search">
            <input
              type="text"
              placeholder={t("admin.singer.input")}
              onKeyUp={(e) => setSearchInput(e.target.value)}
            />
            <HiSearch />
          </div>
          <div className="addUser">
            <button onClick={()=>navigate("/admin/musics/singers/add-singer")}>{t("admin.singer.button")}</button>
          </div>
        </div>
      </header>
      <div className="content">
        <InfiniteScroll
          dataLength={singers.length} // Malumotlar massivi uzunligi
