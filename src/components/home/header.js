...
export default function MainHeader() {
    ...
    useEffect(() => {
        if (pathname && pathname.split("/")[2]) setSearch(pathname.split("/")[2].replaceAll("%20", " "))
    }, [])

    return (
        <header id="main-header">
            <div className="naviga">
                <Link href={`/${pathname && pathname.split('/')[1]}`} name="back"><FiChevronLeft /></Link>
                <div className="path">
                    <p className={pathname && pathname.split('/').length == 2 ? "active" : ""}><span className={pathname && pathname.split('/')[1].startsWith("@") ? "username" : "page"}>{pathname && pathname.split('/')[1]}</span></p>
                    {pathname && pathname.split('/').map((p, i) => (
            ...
            <div className="manage">
                <div className="search">
                    <input type="text" placeholder="Search music, artist, album..." onKeyDown={handleSearch} onChange={(e) => setSearch(e.target.value)} value={search} />
                ...
                <Link href={"/notification"} className={pathname && pathname.split('/')[1] == "notification" ? "active" : ""} name="notification">
